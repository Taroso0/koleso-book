...
export function ThemeNode({
  ...
  // кегль подписи растёт со степенью по √ (как радиус, Шаг 3.4): линейный потолок
  // уравнивал «Человек» и «Душу» (19/19), √ разводит вершину (19.3/18.0). Счётчик
  // опускаем пропорционально кеглю, чтобы десцендеры («у», «Д») не легли на «· N»
  // у крупных тем (§ фикс ①).
  const labelSize = Math.min(19.5, 8.5 + 2.3 * Math.sqrt(degree));
